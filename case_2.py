...
Original file is located at
    https://colab.research.google.com/drive/1kApEhE8MFOSWoxPIuWZlzjdAOnBNCgzr
"""
...
class MicrogridEnv(gym.Env):
    def __init__(self, consumption_data, production_data, spotmarket_data, n_past=5):
        ...
        # State vector with added zeta_s
        self.state = np.array([self.s_B, self.s_H2, self._calculate_zeta_s(self.t)] + self.past_consumption + self.past_production, dtype=np.float32)

        # Define action and observation space with added zeta_s
        low_bounds = [0] * (3 + 2 * self.n_past)  # Battery, Hydrogen, zeta_s, past consumption, and past production
        high_bounds = [np.inf] * (3 + 2 * self.n_past)
    ...
    def step(self, action):
        ...
        #Update the state with added zeta_s
        zeta_s = self._calculate_zeta_s(self.t)
        self.state = np.concatenate(([self.s_B, self.s_H2, zeta_s], self.past_consumption, self.past_production))
    ...
    def reset(self):
        ...
        self.past_production = [0]*self.n_past
         ...
         # Update the state with added zeta_s
        zta_s = self._calculate_zeta_s(self.t)e
        self.state = np.array([self.s_B, self.s_H2, zeta_s] + self.past_consumption + self.past_production, dtype=np.float32)
...
plt.title('Reward per Episode during Training for Case 2')
plt.grid(True)
plt.show()
...
agent.load('models/microgrid_doubledqn_weights.h5')
...
print(f"Evaluation score on summer test data for case 2: {summer_score}")
print(f"Evaluation score on winter test data for case 2: {winter_score}")
...
generate_specific_graph(test_agent, summer_test_env, state_size, "Policy During Summer (Test Data) for case 2")
generate_specific_graph(test_agent, winter_test_env, state_size, "Policy During Winter (Test Data)for case 2")