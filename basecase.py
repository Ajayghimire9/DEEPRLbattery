...
Original file is located at
    https://colab.research.google.com/drive/1aS778j0_sKvQxLVRwgUxF6_TMMRdIO2G
"""
...
class MicrogridEnv(gym.Env):
    def __init__(self, consumption_data, production_data, spotmarket_data, n_past=5):
        ...
        # State vector
        self.state = np.array([self.s_B, self.s_H2] + self.past_consumption + self.past_production, dtype=np.float32)



        # Define action and observation space
        low_bounds = [0] * (2 + 2 * self.n_past)  # Battery, Hydrogen, past consumption, and past production
        high_bounds = [np.inf] * (2 + 2 * self.n_past)
...
for i in range(10):
    ...
    print(f"After action {i}, new state: {state}, reward: {reward}, done: {done}")
...
# Run a simple random policy for 10 episodes for summer_env
for episode in range(10):
    state = summer_env.reset()
    done = False
    while not done:
        # Select a random action
        action = summer_env.action_space.sample()
        # Apply the action to the environment
        next_state, reward, done, _ = summer_env.step(action)

# Run a simple random policy for 10 episodes for winter_env
for episode in range(10):
    state = winter_env.reset()
    done = False
    while not done:
        # Select a random action
        action = winter_env.action_space.sample()
        # Apply the action to the environment
        next_state, reward, done, _ = winter_env.step(action)
...
agent.load('models/microgrid_dqn_weights.h5')
...
print(f"Evaluation score on summer test data: {summer_score}")
print(f"Evaluation score on winter test data: {winter_score}")
...
generate_specific_graph(test_agent, summer_test_env, state_size, "Policy During Summer (Test Data)")
generate_specific_graph(test_agent, winter_test_env, state_size, "Policy During Winter (Test Data)")